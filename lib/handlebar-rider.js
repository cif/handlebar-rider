...
(function(){
  ...
  var precompile = function(data) {
    if( data == "" || data == null || ( data.hasOwnProperty("length") && data.length > 0 ) )
      return "";
    else
 ...
 var readAndCompile = function(dir, watch){
    ...
    readTemplatesDirectory(dir, function(err, files) {
      ...
      if( files == null || files == "" || ( files.hasOwnProperty("length") && files.length <= 0 ) ) {
        console.log(color.red + '[handlebar-rider] There are no files for Handlebar-Rider to compile. Did you run the command correctly? ' + color.reset + "\n");
        return false;
      }
