    ...
    try {  
      ...
      async.each(rider.templates, function(template, nextTemplate) {
        ...
        fs.exists(rider.out, function(exists) {
          if(exists) {
            fs.writeFileSync(rider.out, output, 'utf8', function(err) {
              if(err)
                throw error("Unable to write the output file! Make sure your permissions are correct and try again!", err);
              else
