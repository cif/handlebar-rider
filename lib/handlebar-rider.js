...
(function(){
  ...
  var fileHasChanged = function(event, filename){
      ...
      scli.log(color.blue + '[handlebar-rider] change detected to file "' + filename + '"' + color.reset);
      compileTemplates();
  ...
  var directoryHasChanged = function(event, filename){
      ...
      scli.log(color.blue + '[handlebar-rider] new or removed file "' + filename + '" detected, recompiling' + color.reset);
  ...
  var compileTemplates = function(){
    ...
    try {  
        ...
        scli.log(color.red + '[handlebar-rider] Compile Failure while processing "' + filename + '"' + color.reset);
      ...
      try {
        ...
        scli.log(color.green + '[handlebar-rider] compiled ' + processedFiles.length + ' templates in "' + rider.in + '" to "' + rider.out + '"' + color.reset);
        
      } catch(e){
      
          scli.log(color.red + '[handlebar-rider] ERROR! destination file or directory does not exist.' + color.reset + '\n:' + e);
 ...
 var readAndCompile = function(dir, watch){
    ...
    readTemplatesDirectory(dir, function(err, files) {
      ...
      files = files || "";
      if(files == "") {
        scli.log(color.red + '[handlebar-rider] There are no files for Handlebar-Rider to compile. Did you run the command correctly? ' + color.reset + "\n");
        return false;
      }
     ...
     try {
       ...
       for(var f = 0; f < files.length; f++){
        ...
        scli.log(color.red + '[handlebar-rider] ERROR! check to be sure your directories exist. ' + color.reset  + "\n" + e + "\n");
        scli.log('[handlebar-rider] input directory: ' + dir);
        scli.log('[handlebar-rider] destination file: ' + rider.out + "\n");
        if(rider.help)
          scli.log(rider.help)
...
exports.run = function(){ 
  ...
  if(rider.watch) 
    scli.log(color.yellow + '[handlebar-rider] watching template directory ' + dir  + color.reset);
...
exports.watch = function(){
  ...
  scli.log(color.yellow + '[handlebar-rider] watching template directory ' + rider.in  + color.reset);
  rider.watch = true;
